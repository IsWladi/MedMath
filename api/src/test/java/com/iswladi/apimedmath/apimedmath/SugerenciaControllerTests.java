...
class SugerenciaControllerTests {
	...
	void debeRetornarSugerenciaSegunHoraInicioTomaConfigurada() throws Exception {
        String jsonContent = "{ \"cadaCuantoHoras\": 12, \"porCuantosDias\": 5 }";
		this.mockMvc.perform(get("/obtener/sugerencia/horarios?horaInicioDia=09:00")
            ...
            .andExpect(jsonPath("$.cadaCuantoHoras").value(12))
            .andExpect(jsonPath("$.cuantasVecesTomarPorDia").value(2))
            .andExpect(jsonPath("$.sugerenciaHorarios[0][0]").value("09:00 am"))
            .andExpect(jsonPath("$.sugerenciaHorarios[0][1]").value("21:00 pm"))
            .andExpect(jsonPath("$.sugerenciaHorarios[1][0]").value("10:00 am"))
            .andExpect(jsonPath("$.sugerenciaHorarios[1][1]").value("22:00 pm"))
            .andExpect(jsonPath("$.sugerenciaHorarios[2][0]").value("11:00 am"))
            .andExpect(jsonPath("$.sugerenciaHorarios[2][1]").value("23:00 pm"))
            .andExpect(jsonPath("$.sugerenciaHorarios[3][0]").value("12:00 pm"))
            .andExpect(jsonPath("$.sugerenciaHorarios[3][1]").value("00:00 am"));
